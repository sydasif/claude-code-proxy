...
class Settings(BaseModel):
    ...
    # Proxy settings
    config_file: str = Field(default="config.yaml", description="Path to the LiteLLM config file")
    port: int = Field(default=3455, ge=1024, le=65535, description="Port for the proxy to listen on")
    host: str = Field(default="0.0.0.0", description="Host for the proxy to bind to")

    # Authentication settings
    creds_path: Optional[str] = Field(default=None, description="Path to the credentials file")
    api_key_env_var: str = Field(default="QWEN_API_KEY", description="Environment variable name for API key")

    # Retry settings
    max_retries: int = Field(default=3, ge=0, le=10, description="Maximum number of retry attempts")
    retry_delay: float = Field(default=5.0, ge=0.1, le=60.0, description="Delay between retries in seconds")
